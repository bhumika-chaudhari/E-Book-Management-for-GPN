...
$sqlYears = "SELECT DISTINCT year FROM notes_civil WHERE department = 'Civil'";
...
$sqlSubjects = "SELECT DISTINCT subject FROM notes_civil WHERE department = 'Civil' AND year = ?";
...
$sqlBooks = "SELECT year, title, description, pdf_path FROM notes_civil WHERE department = 'Civil'";
if ($selectedYear) {
    $sqlBooks .= " AND year = ?";
}
if ($selectedSubject) {
...
<head>
    ...
    <title>Notes Library For Civil Department</title>
...
<body style=' background-color: #eaddc3;'>
    ...
    <header>
        <h1>Notes Library For Civil Department</h1>
