...
$sqlYears = "SELECT DISTINCT year FROM notes_computer WHERE department = 'Computer'";
...
$sqlSubjects = "SELECT DISTINCT subject FROM notes_computer WHERE department = 'Computer' AND year = ?";
...
$sqlBooks = "SELECT year, title, description, pdf_path FROM notes_computer WHERE department = 'Computer'";
if ($selectedYear) {
    $sqlBooks .= " AND year = ?";
}
if ($selectedSubject) {
...
<body style=' background-color: #eaddc3;'>
    ...
    <header>
        <h1>Notes Library For Computer Department</h1>
