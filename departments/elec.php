...
$sqlYears = "SELECT DISTINCT year FROM notes_electrical WHERE department = 'Electrical'";
...
$sqlSubjects = "SELECT DISTINCT subject FROM notes_electrical WHERE department = 'Electrical'AND year = ?";
...
$sqlBooks = "SELECT year, title, description, pdf_path FROM notes_electrical WHERE department = 'Electrical'";
if ($selectedYear) {
    $sqlBooks .= " AND year = ?";
}
if ($selectedSubject) {
...
<head>
    ...
    <title>Notes Library For Computer Department</title>
...
<body style=' background-color: #eaddc3;'>
    ...
    <header>
        <h1>Notes Library For Electrical Department</h1>
