...
$sqlYears = "SELECT DISTINCT year FROM notes_entc WHERE department = 'Entc'";
...
$sqlSubjects = "SELECT DISTINCT subject FROM notes_entc WHERE department = 'Entc' AND year = ?";
...
$sqlBooks = "SELECT year, title, description, pdf_path FROM notes_entc WHERE department = 'Entc'";
if ($selectedYear) {
    $sqlBooks .= " AND year = ?";
}
if ($selectedSubject) {
...
<head>
    ...
    <title>Notes Library For Entc Department</title>
...
<body style=' background-color: #eaddc3;'>
    ...
    <header>
        <h1>Notes Library For Entc Department</h1>
