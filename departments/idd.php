...
$sqlYears = "SELECT DISTINCT year FROM notes_idd WHERE department = 'Interior Design'";
...
$sqlSubjects = "SELECT DISTINCT subject FROM notes_idd WHERE department = 'Interior Design' AND year = ?";
...
$sqlBooks = "SELECT year, title, description, pdf_path FROM notes_idd WHERE department = 'Interior Design'";
if ($selectedYear) {
    $sqlBooks .= " AND year = ?";
}
if ($selectedSubject) {
...
<head>
    ...
    <title>Notes Library For Interior Design Department</title>
...
<body style=' background-color: #eaddc3;'>
    ...
    <header>
        <h1>Notes Library For Interior Design Department</h1>
