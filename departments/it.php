...
$sqlYears = "SELECT DISTINCT year FROM notes_if WHERE department = 'IT'";
...
$sqlSubjects = "SELECT DISTINCT subject FROM notes_if WHERE department = 'IT' AND year = ?";
...
$sqlBooks = "SELECT year, title, description, pdf_path FROM notes_if WHERE department = 'IT'";
if ($selectedYear) {
    $sqlBooks .= " AND year = ?";
}
if ($selectedSubject) {
...
<head>
    ...
    <title>Notes Library For IT Department</title>
...
<body style=' background-color: #eaddc3;'>
    ...
    <header>
        <h1>Notes Library For Information Technology Department</h1>
