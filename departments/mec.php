...
$sqlYears = "SELECT DISTINCT year FROM notes_mechanical WHERE department = 'Mechanical'";
...
$sqlSubjects = "SELECT DISTINCT subject FROM notes_mechanical WHERE department = 'Mechanical' AND year = ?";
...
$sqlBooks = "SELECT year, title, description, pdf_path FROM notes_mechanical WHERE department = 'Mechanical'";
if ($selectedYear) {
    $sqlBooks .= " AND year = ?";
}
if ($selectedSubject) {
...
<head>
    ...
    <title>Notes Library For Mechanical Department</title>
...
<body style=' background-color: #eaddc3;'>
    ...
    <header>
        <h1>Notes Library For Mechanical Department</h1>
