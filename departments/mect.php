...
$sqlYears = "SELECT DISTINCT year FROM notes_mect WHERE department = 'Mechatronics'";
...
$sqlSubjects = "SELECT DISTINCT subject FROM notes_mect WHERE department = 'Mechatronics' AND year = ?";
...
$sqlBooks = "SELECT year, title, description, pdf_path FROM notes_mect WHERE department = 'Mechatronics'";
if ($selectedYear) {
    $sqlBooks .= " AND year = ?";
}
if ($selectedSubject) {
...
<head>
    ...
    <title>Notes Library For Mechatronics Department</title>
...
<body style=' background-color: #eaddc3;'>
    ...
    <header>
        <h1>Notes Library For Mechatronics Department</h1>
