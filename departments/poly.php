...
$sqlYears = "SELECT DISTINCT year FROM notes_poly WHERE department = 'Polymer'";
...
$sqlSubjects = "SELECT DISTINCT subject FROM notes_poly WHERE department = 'Polymer' AND year = ?";
...
$sqlBooks = "SELECT year, title, description, pdf_path FROM notes_poly WHERE department = 'Polymer'";
if ($selectedYear) {
    $sqlBooks .= " AND year = ?";
}
if ($selectedSubject) {
...
<head>
    ...
    <title>Notes Library For Polymer Department</title>
...
<body style=' background-color: #eaddc3;'>
    ...
    <header>
        <h1>Notes Library For Polymer Department</h1>
